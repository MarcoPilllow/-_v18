...
DEFAULT_CONCEPT_STOCKS = {"🔥 航運三雄": "2603, 2609, 2615", "🤖 AI 伺服器": "2382, 3231, 2376"}
...
st.subheader("2. 查詢區間")
presets = [[("1周", 7), ("2周", 14), ("3周", 21), ("1月", 30)], [("6周", 42), ("2月", 60), ("1季", 90), ("半年", 182)]]
    ...
    if not targets:
        ...
        for idx, sid in enumerate(targets):
            ...
            if df is not None and not df.empty:
                ...
                final_summary[sid] = {"name": sname, "tot": df['total_net'].sum(), "f": df['f_net'].sum(), "it": df['it_net'].sum(), "d": df['d_net'].sum()}
            progress_bar.progress((idx+1)/len(targets))
        
        progress_bar.empty()
        if results_list:
            st.session_state.full_df, st.session_state.summary, st.session_state.targets = pd.concat(results_list), final_summary, targets
            st.session_state.has_run = True

if st.session_state.get('has_run'):
    selected_label = st.radio("切換數據", ["三大法人總和", "外資", "投信", "自營商"], horizontal=True, label_visibility="collapsed")
    ...
    for sid in st.session_state.targets:
        ...
        # 【修正顯示】這裡決定了圖表的大標題，現在只會出現：【股票名稱 (代碼)】
        fig.update_layout(title=f"【{name} ({sid})】{selected_label} (張)", template="plotly_dark", height=300, margin=dict(l=10, r=10, t=50, b=10))

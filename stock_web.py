...
with st.sidebar:
    ...
    if selected_list != "請選擇...": current_val = st.session_state.custom_lists[selected_list]
    stock_input = st.text_input("1. 目前查詢股票代號", value=current_val)
    new_name = st.text_input("💾 儲存目前組合", placeholder="例如: 我的最愛")
    c1, c2 = st.columns(2)
    with c1:
        if st.button("💾 儲存"):
    ...
    if total_tasks == 0:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for future in as_completed(future_to_date):
                ...
                avg = (time.time() - start_exec) / completed
                eta = int(avg * (total_tasks - completed))
                status_area.markdown(f'<div class="status-box"><b>🏥 加速掃描中：[{completed}/{total_tasks}]</b><br>正在處理：{date_obj.strftime("%Y-%m-%d")}</div>', unsafe_allow_html=True)
        ...
        if results_map:
            ...
            for stock in targets:
                ...
                fig.update_layout(
                    ...
                    xaxis=dict(tickformat="%m/%d", type='category')
                )

import streamlit as st
import requests
import pandas as pd
import datetime
import plotly.graph_objects as go
import json
import os
import re

# 1. 基礎環境設定
st.set_page_config(page_title="台股三大法人籌碼變化", layout="centered")

# CSS 優化：隱藏殘影並美化 UI
st.markdown("""
    <style>
    #MainMenu {visibility: hidden;}
    footer {visibility: hidden;}
    .stApp [data-testid="stToolbar"] {display:none;}
    .stButton button { width: 100%; border-radius: 5px; }
    .status-box { background-color: #1e1e1e; padding: 12px; border-radius: 8px; border-left: 5px solid #ff4b4b; color: #ffffff; }
    </style>
    """, unsafe_allow_html=True)

# --- 2. 聚財網數據抓取與解析 (加速引擎) ---
def fetch_wearn_data(stock_id):
    """從聚財網抓取三大法人歷史數據"""
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'}
    
    urls = {
        'f': f"https://stock.wearn.com/a50m.asp?stockid={stock_id}", # 外資
        'it': f"https://stock.wearn.com/b50m.asp?stockid={stock_id}", # 投信
        'd': f"https://stock.wearn.com/c50.asp?stockid={stock_id}"   # 自營商
    }
    
    combined_df = None
    
    try:
        for key, url in urls.items():
            resp = requests.get(url, headers=headers, timeout=15)
            resp.encoding = 'big5' # 聚財網使用 Big5 編碼
            
            # 解析表格 (通常是頁面中唯一一個 class 為 stockTable 的表格)
            dfs = pd.read_html(resp.text)
            # 尋找包含「日期」的表格
            target_df = None
            for d in dfs:
                if '日期' in str(d.columns):
                    target_df = d
                    break
            
            if target_df is not None:
                # 取得日期與買賣超欄位
                # 外資(a50m)買賣超在第4欄, 投信/自營(b50m/c50)在第3欄
                col_idx = 3 if key == 'f' else 2
                temp = target_df.iloc[:, [0, col_idx]].copy()
                temp.columns = ['date_raw', key]
                
                # 轉換民國年為西元年 (113/03/15 -> 2024/03/15)
                def convert_date(d_str):
                    try:
                        pts = d_str.split('/')
                        return datetime.date(int(pts[0])+1911, int(pts[1]), int(pts[2]))
                    except: return None
                
                temp['date'] = temp['date_raw'].apply(convert_date)
                temp = temp.dropna(subset=['date']).drop(columns=['date_raw'])
                
                if combined_df is None:
                    combined_df = temp
                else:
                    combined_df = pd.merge(combined_df, temp, on='date', how='outer')
        
        if combined_df is not None:
            combined_df = combined_df.fillna(0)
            # 確保數字格式 (移除可能存在的逗號)
            for col in ['f', 'it', 'd']:
                combined_df[col] = combined_df[col].astype(str).str.replace(',', '').astype(float).astype(int)
            
            combined_df['tot'] = combined_df['f'] + combined_df['it'] + combined_df['d']
            return combined_df.sort_values('date')
    except Exception as e:
        return None
    return None

# --- 3. 股票清單管理 ---
LIST_FILE = "stock_lists.json"
def load_lists():
    if os.path.exists(LIST_FILE):
        try:
            with open(LIST_FILE, "r", encoding="utf-8") as f: return json.load(f)
        except: return {"常用航運": "2603, 2609, 2615"}
    return {"常用航運": "2603, 2609, 2615"}

if 'custom_lists' not in st.session_state:
    st.session_state.custom_lists = load_lists()
if 'analysis_results' not in st.session_state:
    st.session_state.analysis_results = None

# --- 4. 側邊欄 UI ---
with st.sidebar:
    st.header("📂 股票清單管理")
    list_names = list(st.session_state.custom_lists.keys())
    selected_list = st.selectbox("讀取查詢組合", ["請選擇..."] + list_names)
    
    current_stocks = "2603, 2609, 2615, 2605, 2606, 2637"
    if selected_list != "請選擇...":
        current_stocks = st.session_state.custom_lists[selected_list]
    
    stock_input = st.text_input("目前查詢代號", value=current_stocks)
    
    st.divider(); st.header("📅 快速區間")
    presets = [[("1周", 7), ("2周", 14), ("3周", 21), ("1月", 30)], [("2月", 60), ("1季", 90), ("半年", 182), ("1年", 365)]]
    
    if 'start_date' not in st.session_state:
        st.session_state.start_date = datetime.date.today() - datetime.timedelta(days=14)
        st.session_state.range_label = "自定義"

    for row in presets:
        cols = st.columns(4)
        for i, (label, days) in enumerate(row):
            if cols[i].button(label):
                st.session_state.start_date = datetime.date.today() - datetime.timedelta(days=days-1)
                st.session_state.range_label = label
                st.rerun()

    start_d = st.date_input("開始日期", st.session_state.start_date)
    end_d = st.date_input("結束日期", datetime.date.today())
    run_btn = st.button("🚀 執行籌碼分析", type="primary", use_container_width=True)

# --- 5. 核心執行邏輯 ---
st.title("📈 台股三大法人籌碼變化")

if run_btn:
    targets = [s.strip() for s in stock_input.replace('，', ',').split(',') if s.strip()]
    results = {}
    progress = st.progress(0)
    
    for i, sid in enumerate(targets):
        st.write(f"🏥 正在調閱股票數據: {sid}...")
        df = fetch_wear_data(sid)
        if df is not None:
            # 根據使用者選取的日期區間過濾
            mask = (df['date'] >= start_d) & (df['date'] <= end_d)
            results[sid] = df.loc[mask]
        progress.progress((i + 1) / len(targets))
    
    st.session_state.analysis_results = results
    st.session_state.last_query_info = {"start": start_d, "end": end_d, "label": st.session_state.range_label}
    st.rerun()

# --- 6. 顯示圖表與 Toggle 功能 ---
if st.session_state.analysis_results:
    res = st.session_state.analysis_results
    info = st.session_state.last_query_info
    
    st.success(f"✅ 診斷完成！已取得 {len(res)} 檔股票數據")
    
    st.divider()
    # Toggle 按鈕置於圖表上方
    view_mode = st.radio("📊 選擇顯示維度", ["三大法人總計", "外資", "投信", "自營商"], horizontal=True)
    mode_map = {"三大法人總計": "tot", "外資": "f", "投信": "it", "自營商": "d"}
    target_col = mode_map[view_mode]

    for sid, df in res.items():
        if df.empty:
            st.warning(f"股票 {sid} 於該區間內無數據")
            continue
            
        fig = go.Figure()
        # 標籤優化：正負號顯示 + 移除小數點
        fig.add_trace(go.Bar(
            x=df['date'],
            y=df[target_col],
            marker_color=['#ef5350' if x >= 0 else '#66bb6a' for x in df[target_col]],
            name=view_mode,
            hovertemplate="日期: %{x|%m/%d}<br>張數: %{y:+.0f} 張<extra></extra>"
        ))
        
        fig.update_layout(
            title=f"【{sid}】{view_mode} 籌碼 (張)",
            template="plotly_dark",
            margin=dict(l=10, r=10, t=50, b=10), height=350,
            xaxis=dict(
                type='category', 
                tickformat='%m/%d', 
                tickangle=-45,
                tickfont=dict(size=10)
            ),
            yaxis=dict(title="張數")
        )
        st.plotly_chart(fig, use_container_width=True)

    # 深度報告
    st.divider(); st.subheader("📋 深度會診報告")
    report = f"【期間：{info['start']} ~ {info['end']}】\n【區間：{info['label']}】\n" + "═"*45 + "\n\n"
    
    for title, key in [("[三大法人總和]", 'tot'), ("[外資]", 'f'), ("[投信]", 'it'), ("[自營商]", 'd')]:
        report += f"{title}\n"
        for sid, df in res.items():
            if not df.empty:
                val = df[key].sum()
                report += f"{sid}: {val:+,} 張\n"
        report += "\n"
    st.code(report, language="text")
